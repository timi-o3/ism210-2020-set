 ...
 const Component6: React.FC = ()  => {
    ...
    useEffect(() => {
        ...
        {/** Below, let's use usual onChangeText */} <View>
        <TextInput onChangeText={(text)=>{userIdInput = text}}
        placeholder="UserId (1 to 10) here" /> </View>
        <View>
        <Button title="Fetch User" onPress={()=>{fetchData(`https://jsonplaceholder.typicode.com/users/${userIdInput}`)}}/>
        </View>
        {/**Below, let's use onSubmitEditing to invoke fetchData. Fetches when enter key is pressed */} <View>
        <TextInput onSubmitEditing={(event)=>{fetchData(`https://jsonplaceholder.typicode.com/users/${event.nativeEvent.text}`)}} placeholder="Enter UserId (1 to 10) here. Press enter when done"
